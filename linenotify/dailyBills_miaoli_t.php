...
// Entry
	...
	try {
		...
		if ($conn_res["status"] == "true") {
			$sql = "SELECT * FROM `log_history4user` WHERE DATE(stop_time)='$pre_date' AND station_name LIKE '%$section%' ORDER BY create_date ASC;";
			...
			$sql = "SELECT 
					...
					WHERE start_time IS NOT NULL AND stop_time IS NOT NULL AND DATE(stop_time)='$pre_date' AND station_name LIKE '%$section%' 
