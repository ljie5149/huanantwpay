<?php
	...
	$jtg_bills_token_group_id = "9d26baa1fb";
	$jtg_charge_bills_token_group_id = "9d26baa1fb";
...
// Entry
	...
	try {
		...
		$section = "苗栗"; // 竹東：縣民廣場平面停車場	苗栗：自治路充電站	南投：停五停車場
		if ($conn_res["status"] == "true") {
			...
			$DC_kwh = 0.0; $AC_kwh = 0.0; 
			$charge_count = 0; $total_amount = 0; $total_kwh = 0.0; $calc_seconds = 0; $paied_count = 0; $paied = 0; $unPay_count = 0; $unPay = 0; $free_count = 0;
			$arr_charge_count = []; $arr_total_amount = []; $arr_total_kwh = []; $arr_calc_seconds = []; 
			$arr_paied_count = []; $arr_paied = []; $arr_unPay_count = []; $arr_unPay = [];
			$nid = 0; $idx = 0; $areas = [];
			$message .= "\n統計日期：$pre_date\n";
			if ($result = mysqli_query($link, $sql)) {
				if (mysqli_num_rows($result) > 0) {
					while($row = mysqli_fetch_array($result)) {
						...
						if (strpos($row['charger_name'], "快充") !== false) {
							$DC_kwh += doubleval($row['kwh']); // 度數
						} else {
							$AC_kwh += doubleval($row['kwh']); // 度數
						}
						...
						// 區域加總
						$area_stats[$area]['charge_count']++;
						$area_stats[$area]['calc_seconds'] += getTwoDateTimeSeconds($row['start_time'], $row['stop_time']);
						$area_stats[$area]['kwh'] += doubleval($row['kwh']);
						$area_stats[$area]['amount'] += intval($row['price']);
						if (strpos($row['charger_name'], "快充") !== false) {
							$area_stats[$area]['DC_kwh'] += doubleval($row['kwh']); // 度數
						} else {
							$area_stats[$area]['AC_kwh'] += doubleval($row['kwh']); // 度數
						}

						
						if (intval($row['price']) > 0) {
							if ($row['jtg_pay_status'] == "1") {
								$area_stats[$area]['paied'] += intval($row['price']);
								$area_stats[$area]['paied_count']++;
							} else {
								$area_stats[$area]['unPay'] += intval($row['price']);
								$area_stats[$area]['unPay_count']++;
							}
						} else {
							$area_stats[$area]['free_count']++; // 無需付款
						}
					...
					foreach ($areas as $area_name) {
						...
						$area_time = secondsToTime($stats['calc_seconds']);
						$message .= "\n\n▶ 單元：$area_name";
						$message .= "\n充電車次：{$stats['charge_count']}";
						$message .= "\n總充電時間：$area_time";
						// $message .= "\n使用度數：{$stats['kwh']}";
						$message .= "\n慢充樁使用度數 :{$stats['AC_kwh']}";
						$message .= "\n快充樁使用度數 :{$stats['DC_kwh']}";
						$message .= "\n營業額：{$stats['amount']} 元";
						$message .= "\n已付款：{$stats['paied_count']} 筆，共 {$stats['paied']} 元";
						$message .= "\n未付款：{$stats['unPay_count']} 筆，共 {$stats['unPay']} 元";
						$message .= "\n無需付款：{$stats['free_count']} 筆";
					}
				} else {
					$message .= "\n場域 :$section\n";
					$dest_time = secondsToTime($calc_seconds);
			...
			if ($result = mysqli_query($link, $sql)) {
				...
				if (mysqli_num_rows($result) > 0) {
					...
					// echo $json_data."\n";
					$msg_ret   = jackyCallAPI($error, "https://miaoparking.jotangi.net/jtgmonitor4miaoli_5/api/JTG_chargeAccount.php", $json_data, "POST");
				} else {
